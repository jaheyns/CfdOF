...
__title__ = "TaskPanelFemMaterial"
...
class TaskPanelFemMaterial:
    '''The editmode TaskPanel for FemlMaterial objects'''
    def __init__(self, obj):
        ...
        self.form = FreeCADGui.PySideUic.loadUi(os.path.dirname(__file__) + os.path.sep + "TaskPanelFemMaterial.ui")
        ...
        # newly added code for FemMaterial
        self.form.comboCategory.clear()
        self.form.comboCategory.addItems(["Solid", "Fluid"])
        QtCore.QObject.connect(self.form.input_kinematic_viscosity, QtCore.SIGNAL("valueChanged(double)"), self.viscosity_changed)
        self.form.list_References.connect(self.form.combo_category, QtCore.SIGNAL("currentIndexChanged(int)"), self.category_changed)
        
        if self.obj.Category == "Solid":
            self.form.combo_category.setCurrentIndex(0)
            #self.form.tabFluidic.setVisble(False)
            #self.form.tabMechanical.setVisble(True)
            
            #def import_mechanical_material():
            ...
            #self.form.tabFluidic.setVisble(True)
            #self.form.tabMechanical.setVisble(False)
            self.form.combo_category.setCurrentIndex(0)
    ...
    def accept(self):
        ...
        #macro recording should be added here
        if self.has_equal_references_shape_types():
            if self.Category == "Solid":
                self.obj.Material = self.material
            elif self.Category == "Fluid":
                self.KinematicViscosity = self.form.input_kinematic_viscosity.value()
    ...
    def category_changed(self):
        if self.obj.Category == "Fluid":
            self.tabMechanical.setVisible(False)
            self.tabFluidic.setVisible(True)
        elif self.obj.Category == "Solid":
            self.tabMechanical.setVisible(True)
            self.tabFluidic.setVisible(False)
        else:
            FreeCAD.Console.PrintMessage("Material catogory not recognised\n")
            
    def viscosity_changed(self, value):
        pass
        """
        import Units
        old_viscosity = Units.Quantity(self.obj.KinematicViscosity)
        if old_viscosity != value:
            self.obj.KinematicViscosity = unicode(value) + " m^2/s"
        """
